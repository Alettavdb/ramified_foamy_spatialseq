...
binSize <- args[8]
prefix <- args[9]
#filePNG <- '769BR_D6.RNA.1_10.png';fileSeu <-"769BR_D6.cb.seuobj.rds";binSize <- 1;prefix <- "769BR_D6"
...
xs <- floor(seuobj$coor_x/ratio_png*binSize)
xs <- xs-min(xs)+1
ys <- floor(seuobj$coor_y/ratio_png*binSize)
ys <- ys-min(ys)+1
