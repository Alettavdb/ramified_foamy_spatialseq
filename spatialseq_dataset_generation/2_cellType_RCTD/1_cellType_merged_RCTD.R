## Rscript /data/work/mydata/runRCTD-mergedAll.R /data/work/mydata/Absinta2021_noCI_noNeurons.rds /data/work/mydata/Cellbin_merged_noNA-GM-Don5.rds
args<-commandArgs(T)

# Rscript /data/work/MS_brain/merged/runRCTD_merged.R
library(Seurat)
library(spacexr)
library(ggplot2)
...
scData<- args[1] #"/data/work/mydata/Absinta2021_noCI_noNeurons.rds"
spData<- args[2] #"/data/work/mydata/Cellbin_merged_noGM_noDon5.rds"
...
reference<-Reference(sc.counts, cell_types, nUMI, n_max_cells = 140000)
...
myRCTD<-create.RCTD(puck, reference, max_cores = 10, CELL_MIN_INSTANCE = 10, UMI_min = 1,UMI_min_sigma=1) # for parallel processing, the number of cores used. 1 means no parallel processing.
...
pdf(paste0(spData, "_cell_type_RCTD.pdf"),width=15,height=9)
...
p4<-ggplot(sp.data@meta.data,aes(x=RCTD, y=1, fill=region))+geom_col(position="fill")+scale_y_continuous(labels = scales::percent)+
    scale_fill_manual(values = myCol)+labs(x="Subtypes", y="Proportion", fill="Region", title="Region in subtypes")+
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
# compare RCTD on merged vs previous on single
p6<-ggplot(sp.data@meta.data,aes(x=RCTD, y=1, fill=celltype))+geom_col(position="fill")+scale_y_continuous(labels = scales::percent)+
    scale_fill_manual(values = myCol)+labs(x="RCTD merged", y="Proportion", fill="celltype", title="RCTD in celltype")+
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+facet_grid(.~donor)
grid.arrange(p6)
dev.off()
